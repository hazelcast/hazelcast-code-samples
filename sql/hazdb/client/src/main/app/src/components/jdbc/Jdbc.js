...
class Jdbc extends Component {
    constructor(props) {
        super(props);
        this.state = {
			param: 'Bayern Munich',
			query: 'SELECT * FROM bundesliga WHERE __key = ?',
            message0: '',
            message0_style: {
                'color': 'green',
                'font-weight': 'bold'
    ...
    handleSubmit(e) {
        ...
        setTimeout(() => {
            ...
            var restURL = '/rest/jdbcNoParam?sql=' + this.state.query;
            if (this.state.param.length > 0) {
				if (isNaN(this.state.param)) {
					restURL = '/rest/jdbc1StringParam?sql=' + this.state.query + "&param=" + this.state.param;
				} else {
					restURL = '/rest/jdbc1IntegerParam?sql=' + this.state.query + "&param=" + this.state.param;
				}
			}
	        console.log("Jdbc.js", "handleSubmit()", restURL);

            client({path:restURL}).then(function(response) {
                console.log("Jdbc.js", "handleSubmit()", "response.entity", response.entity);
                 ...
                 <div>
                     <form>
                         ...
                         <input type="text" size="12"
                                id="param" name="param" defaultValue={this.state.param}
                                onChange={this.handleChange}/>
