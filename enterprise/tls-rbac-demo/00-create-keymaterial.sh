...
authorityInfoAccess = OCSP;URI:http://localhost:16975
...
prepareClientKeyMaterialExt() {
  echo "Preparing client key material: $1, Extension: $2, Startdate: $3"
  # Generate a key
  ...
  if [ -z "$3" ]; then
    ...
    openssl ca -config ${OPENSSL_CONF} -startdate ${3}0101000000Z -enddate ${3}1231235959Z -extensions $2 -extfile ${OPENSSL_CONF} -batch -keyfile ca-key.pem -cert ca-cert.pem -out $1-cert.pem -infiles $1-cert.csr
  fi
  exportKeystores $1
}

prepareClientKeyMaterial() {
  prepareClientKeyMaterialExt $1 v3_req
}
...
openssl req -new -x509 -extensions v3_ca -config ${OPENSSL_CONF} -keyform PEM -key ca-key.pem -outform DER -out ca-cert.crt -days 365 -subj '/C=US/O=Hazelcast/CN=Hazelcast CA'
exportKeystores ca

echo ">>> Generate Server key material"
prepareClientKeyMaterial ${HOSTNAME}

echo ">>> Generate Valid clients key material"
