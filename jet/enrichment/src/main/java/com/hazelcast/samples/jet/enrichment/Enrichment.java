...
package com.hazelcast.samples.jet.enrichment;
...
import com.hazelcast.samples.jet.enrichment.datamodel.Product;
import com.hazelcast.samples.jet.enrichment.datamodel.Trade;
...
public final class Enrichment {
    ...
    private Enrichment(HazelcastInstance hz) {
        this.hz = hz;
    }
    ...
    private Pipeline enrichUsingIMap() {
        IMap<Integer, Product> productMap = hz.getMap(PRODUCTS);
        ...
        IMap<Integer, Broker> brokerMap = hz.getMap(BROKERS);
    ...
    private Pipeline enrichUsingReplicatedMap() {
        ReplicatedMap<Integer, Product> productMap = hz.getReplicatedMap(PRODUCTS);
        readLines("products.txt").forEach(e -> productMap.put(e.getKey(), new Product(e.getKey(), e.getValue())));
        System.out.println("Loaded product replicated map:");
        printMap(productMap);

        ReplicatedMap<Integer, Broker> brokerMap = hz.getReplicatedMap(BROKERS);
        readLines("brokers.txt").forEach(e -> brokerMap.put(e.getKey(), new Broker(e.getKey(), e.getValue())));
