...
package com.hazelcast.samples.jet.cogroup;
...
import com.hazelcast.samples.jet.cogroup.datamodel.AddToCart;
import com.hazelcast.samples.jet.cogroup.datamodel.PageVisit;
import com.hazelcast.samples.jet.cogroup.datamodel.Payment;
...
public final class BatchCoGroup {
    ...
    private BatchCoGroup(HazelcastInstance hz) {
        this.hz = hz;
    }
    ...
    private void validateCoGroupResults() {
        IMap<Integer, Tuple3<List<PageVisit>, List<AddToCart>, List<Payment>>> result = hz.getMap(RESULT);
        printImap(result);
    ...
    private void prepareSampleData() {
        IList<AddToCart> addToCartList = hz.getList(ADD_TO_CART);
        IList<Payment> paymentList = hz.getList(PAYMENT);
        IList<PageVisit> pageVisitList = hz.getList(PAGE_VISIT);
